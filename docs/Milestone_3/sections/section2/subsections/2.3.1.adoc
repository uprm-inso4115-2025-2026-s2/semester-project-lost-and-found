...
a `Match` connects a lost report to a found report. This explains the
`create-then-persist` workflow shown below—both report types share a common
...
- The class hierarchy separates *input* (no system fields) from *persisted* (with id, status, timestamps), making the create-then-persist workflow explicit.
